<template>
  ...
  <div>
    <transition-group id="tag_div" name="tags-list" tag="div">
      <span class="tags_special_bt" key="a"  v-bind:class="[view_filter_tags? 'clear_tags_active': 'clear_tags_inactive']" v-on:click="view_filter_tags = !view_filter_tags" title="toggle tag filter">F</span>
      <span class="tags_special_bt" key="b"  v-bind:class="[view_tag_groups? 'clear_tags_active': 'clear_tags_inactive']" v-on:click="view_tag_groups = !view_tag_groups" title="toggle tag group">G</span>
      <template v-if="view_filter_tags">
        <span v-for="tag in selected_tags" v-on:click="removeElement(selected_tags, tag)" class="selected" v-bind:key="tag.name">{{tag.name}}<sup>{{tag.count}}</sup></span>
      </template>
      <span v-for="tag in sorted_tags" v-on:click="tagClicked(tag)" v-bind:key="tag.name" v-bind:class="[tag.group_border, {selected: !view_filter_tags && selected_tags.includes(tag)}]">{{tag.name}}<sup>{{tag.count}}</sup></span>
      <span class="tags_special_bt" key="z" v-bind:class="[selected_tags.length > 0? 'clear_tags_active': 'clear_tags_inactive']" v-on:click="selected_tags = []">✕</span>
    </transition-group>

    <transition-group id="bottom_div" name="bottom-div" tag="div">
      <transition-group id="inspirations_div" name="inspirations-list" tag="div" key="a">
          <div v-for="inspiration in filtered_inspirations" v-bind:key="inspiration.name" class="inspirations-list-item" v-bind:class="{selected_inspiration: inspiration == selected_inspiration}">
          ...
          <template v-else>
            ...
            <a v-if="!inspiration.embed_type" v-bind:href="inspiration.url" target="_blank">{{inspiration.name}}<span style="margin-left:10px;opacity:0.2">⤤</span></a>
            <a v-else href="#/" v-on:click=" if (selected_inspiration == inspiration) { selected_inspiration = null } else { selected_inspiration = inspiration }">{{inspiration.name}}</a>
          </template>
        </div>
      </transition-group>
      <div class="inspiration_container" key="b">
        <div v-if="selected_inspiration && selected_inspiration.embed_type=='bandcamp'">
          <iframe style="border: 0; width: 350px; height: 786px;" v-bind:src="'https://bandcamp.com/EmbeddedPlayer/album='+selected_inspiration.bandcamp_album_id+'/size=large/bgcol=333333/linkcol=ffffff/transparent=true/'" seamless><a v-bind:href="selected_inspiration.url">⤤</a></iframe>
        </div>
        <div v-if="selected_inspiration && selected_inspiration.embed_type=='wikipedia'" v-html="wikipedia_summary" class="wikipedia_embed"></div>
      </div>
    </transition-group>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      loadWikipediaSummary(inspiration) {
        ...
        let url = 'https://en.wikipedia.org/w/api.php?origin=*&format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&titles='+article_name
        this.axios.get(url).then(response => {
          let results = response.data.query.pages
          this.wikipedia_summary = results[Object.getOwnPropertyNames(results)[0]].extract
        })
      }
    },
    watch: {
        ...
        if (this.view_filter_tags) {
          ...
          for (let i=0; i<tags.length; ++i) {
            ...
            for (let j=0; j<inspirations.length; ++j) {
              if (inspirations[j].tags.includes(tags[i].name)) { // TODO - this will have to change when hierarchies are introduced
                tags[i].count += 1
              }
            }
          }
    ...
    mounted() {
      ...
      for (let i=0; i<this.inspirations.length; ++i) {
        for (let j=0; j<this.inspirations[i].tags.length; ++j) {
          ...
          if (k === this.tags.length) { // tag was not found, add it
            this.tags.push({
              name: this.inspirations[i].tags[j],
              count: 1,
              selected: false
            })
          } else { // tag was found, increment its count
            this.tags[k].count++
          }
        }
      }
...
<style scoped>
  ...
  * {
  ...
  .bottom-div-enter, .bottom-div-leave-to {
    opacity: 0;
  ...
  .tags-list-enter, .tags-list-leave-to {
    opacity: 0;
    /* translate: translateY(30px) */
  }
  .tags-list-move {
  ...
  .inspiration_container {
    ...
    position: sticky; /* TODO DOES NOT WORK*/
    top:0;
  }
  ...
  .inspirations-list-enter, .inspirations-list-leave-to {
    opacity: 0;
    /* translate: translateY(30px) */
  }
